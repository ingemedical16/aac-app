...
export function buildSmartSentence(tiles: TileData[], locale: string): string {
  ...
  if (words.length === 1 && feelings.includes(words[0])) {
    switch (locale) {
      case "en": return `I feel ${words[0]}`;
      case "fr": return `Je me sens ${words[0]}`;
      case "ar": return `أشعر بـ ${words[0]}`;
      case "ro": return `Mă simt ${words[0]}`;
    }
  }
  ...
  if (wants.includes(words[0]) && words.length > 1) {
    ...
    switch (locale) {
      case "en": return `I want ${obj}`;
      case "fr": return `Je veux ${obj}`;
      case "ar": return `أريد ${obj}`;
      case "ro": return `Vreau ${obj}`;
    }
  }
  ...
  if (!subjects.includes(words[0]) && wants.includes(words[0])) {
    ...
    switch (locale) {
      case "en": return `I ${words.join(" ")}`;
      case "fr": return `Je ${words.join(" ")}`;
      case "ar": return `أنا ${words.join(" ")}`;
      case "ro": return `Eu ${words.join(" ")}`;
    }
  }
